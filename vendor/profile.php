...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['new_password'])) {
        if (!password_verify($_POST['current_password'] ?? '', $profile['password'])) {
            flash('error','Current password is incorrect.'); header('Location: profile.php'); exit;
        }
        if (strlen($_POST['new_password']) < 6) {
            flash('error','New password must be at least 6 characters.'); header('Location: profile.php'); exit;
        }
        $hash = password_hash($_POST['new_password'], PASSWORD_DEFAULT);
        $pdo->prepare("UPDATE users SET password=? WHERE id=?")->execute([$hash, $uid]);
    }

    $pdo->prepare("UPDATE users SET name=?,phone=?,company=?,address=?,city=?,state=?,gst_number=?,updated_at=NOW() WHERE id=?")
        ->execute([$name,$phone,$company,$address,$city,$state,$gst,$uid]);
    $_SESSION['name'] = $name;
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" value="<?= sanitize($profile['email']) ?>" disabled>
                    <div class="form-text">Email cannot be changed.</div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Company / Business Name</label>
                        <input type="text" name="company" class="form-control" value="<?= sanitize($profile['company']) ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label">GST Number</label>
                        <input type="text" name="gst_number" class="form-control" value="<?= sanitize($profile['gst_number']) ?>" placeholder="22AAAAA0000A1Z5">
                    </div>
                </div>
                <div class="form-group">
